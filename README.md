...
| **Importar de folhas de cálculo** | Interessados, bens e dívidas colados do Excel/Sheets ou de um CSV, com colunas reconhecidas pelo cabeçalho, validação (NIF, IBAN, valores, datas), repetidos assinalados, pré-visualização e «anular». |
| **Impressão** | Ecrãs impressos em papel branco (também no tema escuro), sem menus nem botões, com cabeçalho do escritório e numeração; checklist inteira mesmo com fases recolhidas; resumo do dossier numa só folha A4. |
| **Diagnóstico e integridade** | Página com o estado da aplicação neste dispositivo (versão, service worker, armazenamento, funcionalidades do navegador) e verificação da integridade dos dados — órfãos, anexos perdidos, referências partidas, tarefas repetidas — com reparação registada no histórico; diagnóstico para enviar sem dados pessoais. |
| **Desempenho** | Arranque com 134 kB de JavaScript (comprimido) — o resto carrega a pedido, com pré-carregamento —, orçamento de tamanho verificado em cada publicação, listas longas desenhadas só quando visíveis; 300 dossiers e 18 000 tarefas abrem em menos de meio segundo. |
| **Teclado e acessibilidade** | Atalhos com ajuda («?»), «G» + letra para ir para cada página, quadro da checklist operável por teclado e sem arrastar, separadores ARIA, animações reduzidas e alto contraste (também pelas preferências do sistema e no modo de cores forçadas do Windows). |
| **Qualidade** | 0 violações axe-core (WCAG 2.2 AA) em 32 ecrãs e estados (incluindo alto contraste claro e escuro), divisão de código por rota e separador, fontes só em latino, 314 testes (unitários, componentes com Testing Library e propriedades com fast-check, escala, cobertura com limiar, higiene do código-fonte) e 24 passos E2E com Chrome (telemóvel a 360 e 390 px, teclado e escritório grande) executados antes de cada publicação, com orçamento de tamanho do pacote. |
