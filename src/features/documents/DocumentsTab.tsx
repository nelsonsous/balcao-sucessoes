...
export function DocumentsTab({ c }: { c: CaseRecord }) {
  ...
  const visible = list.filter((d) => filter === 'todos' || d.status === filter);
  ...
  return (
    <div className={cx('stack docs-tab', dragging && 'dragging')} style={{ gap: 14 }} {...dropHandlers}>
      <div className="toolbar">
        ...
        <Button
          icon={Mail}
          onClick={() => openTemplate('email-documentos-falta')}
          disabled={!list.some((d) => (d.status === 'em_falta' || d.status === 'pedido') && clientCanProvide(d))}
          title="Email ao cliente com os documentos que só ele pode fornecer"
        >
          Pedir ao cliente
        </Button>
          ...
          return (
            <Card key={cat.id} className="doc-group">
              ...
              <ul className="doc-rows">
                {items.map((d) => (
                  <li key={d.id} className={cx('doc-row', `ds-${d.status}`)}>
                    <DocStatusMenu d={d} />
                    <button type="button" className="doc-main" onClick={() => setEditing(d)}>
                    ...
                    <div className="doc-actions">
                      {d.fileId ? (
                        <span className="file-chip">
                          ...
                          <button type="button" className="link-btn" onClick={() => void openAttachment(d, 'view')} title={`Abrir ${d.fileName}`}>
                            {d.fileName}
                          </button>
